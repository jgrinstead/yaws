...
\item       \verb+errormod_conn = Module+ ---
              It is possible to set a special module that does error handling
              for client connection errors.  The default for a client read
              error is to send a short message to the error_logger.

              The function \verb+Module:conn_read_error(CliSock, IPPort, Len, Err)+
              will be called. The \verb+Len+ is the number of bytes being
              read from the client.  The \verb+Err+ is the error that has
              occured.

              The function return is ignored as the client process will
              immediately exit.
